...
const Form = ({ useForm, keyNum = 0 }: IForm) => {
  ...
  const getDescription = useCallback((obj: { type: ITransactionType, name: string }) => {
    let s = '';
    if (obj.type === ITransactionType.CTL) s = 'Chi tiền lương cho nhân viên ' + obj.name;
    else if (obj.type === ITransactionType.CTH) s = 'Chi tiền hoa hồng cho nhân viên ' + obj.name;
    else if (obj.type === ITransactionType.CTD) s = 'Chi tiền thưởng đạt doanh số cho nhân viên ' + obj.name;
    return s;
  }, []);
      ...
      <Grid2 container {...dStyles.containerResponsive} mb={3} mx={2}>
        <Grid2 size={8}>
          <Autocomplete
            ...
            store={{
              mode: 'local',
              data: SourceTransactionType.filter(f => f.type === 'C'),
            }}
            {...register("transtype", {
              required: true,
              onChange: (e) => {
                const v = e.target.value;
                setValue("description", getDescription({ type: v, name: empNameRef.current }), { shouldDirty: true });
                if (v) {
        ...
        <Grid2 size={12}>
          <Autocomplete
            ...
            {...register("objectId", {
              required: (transtype === ITransactionType.PMGB || transtype === ITransactionType.PMGT) ? 'Chưa nhập Nhân viên' : false,
              onChange: (e) => {
                const rec = e.target.rec;
                if (rec) {
                  const type = getValues("transtype") as ITransactionType;
                  empNameRef.current = rec.name;
                  setValue("description", getDescription({ type, name: rec.name }), { shouldDirty: true });
                }
              }
            })}
          />
