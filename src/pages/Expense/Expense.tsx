...
const ExpensePage = () => {
  ...
  const { info } = useUser();
  const [currentTab, setCurrentTab] = useState<ITab>(ITab.CTL);
  ...
  const saveRecord = () => {
    handleSubmit(
      (o) => {
        if (o.status !== ITransactionStatus.H) {
          o.status = ITransactionStatus.D;
        }
        ...
        data['details'] = [
          { linenum: 1, date: o.transdate, amount: o.totalamount, notes: '' }
        ];
...
export default memo(ExpensePage);