...
const GridCTHExpense = ({
  pagination,
  handleCellDbClick,
  handleActionClick,
  handleUpdateParams,
}: IGrid) => {
  const { cthRecords, isLoading, cthCount } = useTransaction();
  const { info } = useUser();
  ...
  return (
    <Grid2
      ...
      <DataTable
        ...
        columns={columns}
        rows={cthRecords?.map((record, idx) => ({ ...record, id: idx })) ?? []}
        rowCount={cthCount}
        loading={isLoading}
        paginationModel={pagination}
        handlePagination={(paginationModel) => {
          handleUpdateParams({ id: ITab.CTH, pagination: paginationModel });
        }}
        onSortModelChange={(sortModel) => {
          ...
          handleUpdateParams({ id: ITab.CTH, sort: sorts });
        }}
        ...
        handleItemPaginationClick={(_key) => {
          handleUpdateParams({ id: ITab.CTH, refresh: true });
        }}
      />
    </Grid2>
  );
...
export default memo(GridCTHExpense);
