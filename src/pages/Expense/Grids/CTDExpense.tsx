...
const GridCTDExpense = ({
  pagination,
  handleCellDbClick,
  handleActionClick,
  handleUpdateParams,
}: IGrid) => {
  const { ctdRecords, isLoading, ctdCount } = useTransaction();
  const { info } = useUser();
  ...
  return (
    <Grid2
      ...
      <DataTable
        ...
        columns={columns}
        rows={ctdRecords?.map((record, idx) => ({ ...record, id: idx })) ?? []}
        rowCount={ctdCount}
        loading={isLoading}
        paginationModel={pagination}
        handlePagination={(paginationModel) => {
          handleUpdateParams({ id: ITab.CTD, pagination: paginationModel });
        }}
        onSortModelChange={(sortModel) => {
          ...
          handleUpdateParams({ id: ITab.CTD, sort: sorts });
        }}
        ...
        handleItemPaginationClick={(_key) => {
          handleUpdateParams({ id: ITab.CTD, refresh: true });
        }}
      />
    </Grid2>
  );
...
export default memo(GridCTDExpense);
