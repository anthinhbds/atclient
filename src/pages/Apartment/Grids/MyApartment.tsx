...
import { ITab, MyApartmentColumns } from "./data";
...
const GridMyApartment = ({
  pagination,
  handleCellDbClick,
  handleActionClick,
  handleUpdateParams,
}: IGrid) => {
  const { myApartmentRecord, isLoading, myApartmentCount } = useApartment();
  const { info } = useUser();
  ...
  return (
    <Grid2
      ...
      <DataTable
        ...
        columns={columns}
        rows={myApartmentRecord?.map((record, idx) => ({ ...record, id: idx })) ?? []}
        rowCount={myApartmentCount}
        loading={isLoading}
        paginationModel={pagination}
        handlePagination={(paginationModel) => {
          handleUpdateParams({ id: ITab.MYAPARTMENT, pagination: paginationModel });
        }}
        onSortModelChange={(sortModel) => {
          ...
          handleUpdateParams({ id: ITab.MYAPARTMENT, sort: sorts });
        }}
        ...
        handleItemPaginationClick={(_key) => {
          handleUpdateParams({ id: ITab.MYAPARTMENT, refresh: true });
        }}
      />
    </Grid2>
  );
...
export default memo(GridMyApartment);
