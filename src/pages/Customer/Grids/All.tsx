...
import { ISortItem, ICustomerItem, IAction, EClaimType } from "types";
import {
  GridPaginationModel,
  GridCellParams,
  useGridApiRef,
...
interface IGrid {
  ...
  handleActionClick?: (key: any, row?: ICustomerItem[]) => void;
...
const GridActive = ({
  ...
  const { records, isLoading, totalCount } = useCustomer();
  const { info } = useUser();
