...
import { IAssignmentLog } from "types";
    ...
    <React.Fragment>
      <Dialog
        ...
        <DialogContent sx={{ p: 2 }}>
          <Box>
            <Typography sx={{ fontSize: '16px', fontWeight: 700, color: 'hsl(0,0%,95%)' }}>{`Căn hộ: ${data.apartmentno} - ${data.owner} - ${data.telephone}`}</Typography>
          </Box>
          <Timeline
            sx={{
            ...
            {items && items.map(
              (item, idx) => (
                <TimelineItem key={idx}>
                  ...
                  <TimelineContent color="hsl(0,0%,95%)">{item.user?.name}</TimelineContent>
                </TimelineItem>
              )
            )}
          </Timeline>
