...
const GridPMGFee = ({
  ...
  const { pmgRecords, isLoading, pmgCount } = useTransaction();
  const { info } = useUser();
  ...
  return (
    <Grid2
      ...
      <DataTable
        ...
        columns={columns}
        rows={pmgRecords?.map((record, idx) => ({ ...record, id: idx })) ?? []}
        rowCount={pmgCount}
        loading={isLoading}
        paginationModel={pagination}
        handlePagination={(paginationModel) => {
          handleUpdateParams({ id: ITab.PMG, pagination: paginationModel });
        }}
        onSortModelChange={(sortModel) => {
          ...
          handleUpdateParams({ id: ITab.PMG, sort: sorts });
        }}
        ...
        handleItemPaginationClick={(_key) => {
          handleUpdateParams({ id: ITab.PMG, refresh: true });
        }}
      />
    </Grid2>
  );
...
export default memo(GridPMGFee);
